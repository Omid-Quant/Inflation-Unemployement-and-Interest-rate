...
export default function App() {
  const [activeMetrics, setActiveMetrics] = useState({ inflation: true, unemployment: true, interest: true });
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .recharts-text { fill: #555 !important; font-family: 'IBM Plex Mono', monospace !important; font-size: 11px !important; }
      `}</style>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "48px 24px" }}>
        ...
        <div style={{ marginBottom: 48 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#ff6b35", boxShadow: "0 0 12px #ff6b35", animation: "pulse 2s infinite" }} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginBottom: 40 }}>
          {Object.entries(METRICS).map(([key, meta]) => {
            ...
            return (
              <button
                key={key}
                className="metric-btn stat-card"
                onClick={() => toggleMetric(key)}
                style={{
                  opacity: activeMetrics[key] ? 1 : 0.35,
                  outline: activeMetrics[key] ? `1px solid ${meta.color}22` : "none",
                }}
              >
                <style>{`.stat-card-${key}::before { background: ${meta.color}; }`}</style>
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: "2px", background: activeMetrics[key] ? meta.color : "transparent", borderRadius: "12px 12px 0 0", transition: "background 0.3s" }} />
          ...
          {activeCount === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={340}>
              <LineChart data={ECONOMIC_DATA} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                ...
                <ReferenceLine y={2} stroke="rgba(255,255,255,0.08)" strokeDasharray="4 4" label={{ value: "Fed 2% Target", position: "insideTopRight", fill: "#333", fontSize: 10 }} />
                {Object.entries(METRICS).map(([key, meta]) =>
                  activeMetrics[key] ? (
                    <Line
                      key={key}
